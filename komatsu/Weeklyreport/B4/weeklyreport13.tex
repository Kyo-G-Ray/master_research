...
\Weekly{13}{7}{10}{(水)}{\ 小松　大起}

\section{CNNとは}
\begin{itemize}
\item CNN とは多層 NN の一つで、畳込み NN とも呼ばれる．ネットワーク上に畳込み層とプーリング層の２層を交互に持つネットワークである．$対象とする画像を Wpicsel × Wpicsel の RGB の階調値とし，y^{(k)}_{ij}と表す．$ここで k は RGB の色調を表し k = 1,2,3（N = 3）の値をとる．$畳込み層（i,j）の素子の内部状態 x_{ij} は以下で与えられる．$
\begin{equation}
