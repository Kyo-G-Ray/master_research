...
#データ読み込み　Yは最初の列に配置する
dataframe = pandas.read_csv('aday.csv', usecols=[0,1,2,3])
...
model.add(keras.layers.LSTM(100, input_shape=(testX.shape[1], look_back)))	#shape：変数数、遡る時間数

#2 層の時
#model.add(keras.layers.LSTM(200, input_shape=(testX.shape[1], look_back), return_sequences=True))
#model.add(keras.layers.LSTM(200, input_shape=(testX.shape[1], look_back)))
...
#callback で学習終了条件を追加
call = keras.callbacks.EarlyStopping(monitor='val_loss', patience=100, verbose=1, mode='auto')

history = model.fit(trainX, trainY, epochs=5, batch_size=20, verbose=1, callbacks=[call], validation_split=0.1)
